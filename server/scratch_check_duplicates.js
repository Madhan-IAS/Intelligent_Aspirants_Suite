...
async function main() {
  try {
    ...
    console.log(`Unique topics extracted: ${unique.size}`);
    console.log(`Duplicates: ${topics.length - unique.size}`);

    // Print some duplicate titles
    const counts = {};
    topics.forEach(t => {
      counts[t] = (counts[t] || 0) + 1;
    });
    const duplicates = Object.keys(counts).filter(t => counts[t] > 1);
    console.log('\nFirst 10 duplicate titles:');
    duplicates.slice(0, 10).forEach(t => {
      console.log(`- "${t}" appears ${counts[t]} times`);
    });
