...
async function main() {
  try {
    ...
    // Print all lines that are fully uppercase (ignoring numbers, spaces and punctuation)
    // and see if they correspond to headers.
    console.log('--- POTENTIAL HEADERS ---');
    docTextLines.forEach((line, idx) => {
      const trimmed = line.trim();
      if (trimmed.length > 4 && trimmed.length < 50 && trimmed === trimmed.toUpperCase() && !trimmed.includes('')) {
