/**
 * Seed the UPSC Master Daily Timetable, Checklist Items, and Weekly Schedule.
 * Run: node seedTimetable.js
 */
...
// ═══════════════════════════════════════════
// 1. MASTER DAILY TIMETABLE (5 AM - 11 PM)
// ═══════════════════════════════════════════
...
async function seedAll() {
  try {
    ...
    // --- 1. Timetable ---
    await TimetableSlot.deleteMany({ userId: user._id });
    const slotsWithUser = MASTER_TIMETABLE.map((s, i) => ({ ...s, userId: user._id, order: i }));
    await TimetableSlot.insertMany(slotsWithUser);
    console.log(`📅 Seeded ${slotsWithUser.length} timetable slots`);

    // --- 2. Daily Targets + End-of-Day Checklist ---
    await ChecklistItem.deleteMany({ userId: user._id });
    ...
    console.log(`✅ Seeded ${DAILY_TARGETS.length} daily targets + ${END_OF_DAY_CHECKLIST.length} end-of-day checklist items`);

    // --- 3. Weekly Schedule ---
    await WeeklySchedule.deleteMany({ userId: user._id });
    const weeklyWithUser = WEEKLY_SCHEDULE.map((w, i) => ({ ...w, userId: user._id, order: i }));
    await WeeklySchedule.insertMany(weeklyWithUser);
    console.log(`📆 Seeded ${weeklyWithUser.length} weekly schedule entries`);

    console.log('\n✅ All data seeded successfully!\n');

    // Print summary
    console.log('═══════════════════════════════════════');
    console.log('  UPSC CSE 2027 — Master Timetable');
    console.log('═══════════════════════════════════════');
    slotsWithUser.forEach(s => {
      const icon = s.isStudyBlock ? '📖' : (s.category === 'Break' ? '☕' : '⚡');
      console.log(`  ${icon} ${s.time.padEnd(25)} ${s.activity}`);
    });
    console.log('\n═══════════════════════════════════════');
    console.log('  Daily Targets');
    console.log('═══════════════════════════════════════');
    DAILY_TARGETS.forEach(t => console.log(`  ${t.icon} ${t.label}`));
    console.log('\n═══════════════════════════════════════');
    console.log('  Weekly Special Schedule');
    console.log('═══════════════════════════════════════');
    WEEKLY_SCHEDULE.filter(w => w.isSpecial).forEach(w => console.log(`  🎯 ${w.day}: ${w.task} (${w.duration})`));
